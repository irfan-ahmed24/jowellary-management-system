...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $date = $_POST['Date'] ?? '';
    if( $customer_id && $item_id && $quantity && $total_amount && $date) {
        $stmt = mysqli_query($conn, "INSERT INTO `order` ( Customer_ID, Item_id, Quantity, Total_Amount, Order_Date) VALUES ('$customer_id', '$item_id', '$quantity', '$total_amount', '$date')");
        if ($stmt) {
